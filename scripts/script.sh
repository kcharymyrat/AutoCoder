...
# Check for empty response and log the issue
if [[ -z "$RESPONSE" ]]; then
  echo 'Error: Issue details could not be fetched from GitHub API.'
  exit 1
fi

ISSUE_BODY=$(echo "$RESPONSE" | jq -r '.body' || true)

# Handle potential parsing errors with 'jq' by assigning an empty string
if [[ -z "$ISSUE_BODY" ]]; then
  echo 'Warning: Issue body is empty or not found in the response (parsing error).'
else
  echo "Issue body retrieved successfully."
fi
...
FILES_JSON=$(echo "$RESPONSE" | jq -e '.choices[0].message.content | fromjson' || true)

# Handle potential parsing errors with 'jq' by assigning an empty string
if [[ -z "$FILES_JSON" ]]; then
  echo "Warning: No valid JSON dictionary found in the response or the response was not valid JSON. Please rerun the job."
else
  echo "JSON dictionary extracted successfully."
fi
